	...
	devtools(
		(set, get) => ({
			...
			addPersonAnswer: async (personAnswer: PersonAnswerInsert): Promise<void> => {
				...
				localStorage.setItem("answers", person.answers + 1);
			},
			getPerson: async (dni: string): Promise<void> => {
				set({ isLoading: true }, false, GET_PERSON);
				...
				localStorage.setItem("dni", persons[0].dni);
				localStorage.setItem("answers", persons[0].answers);
			},
			updatePerson: async (person: Person): Promise<void> => {
				set({ isLoading: true }, false, UPDATE_PERSON);
